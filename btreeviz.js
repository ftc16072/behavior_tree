 ...
 * @returns {void}
 */
function loadTree(str, graphElement, codeElement) {
  let tree = parse(str),
    line = tree.line;
  if (tree.error) {
    showError(`Line ${line}: ${tree.error}`, graphElement);
    return;
  }
  d3.select(codeElement).html(str);
...
function main(graphElement, codeElement) {
  ...
  treeInput.addEventListener('change', function () {
    ...
    reader.onload = function (e) {
      loadTree(e.target.result, graphElement, codeElement);
    };
  ...
  codeUpdate.addEventListener('click', function () {
    ...
    loadTree(str, graphElement, codeElement)
  }, false);
  ...
  loadTree(SAMPLE_TREE, graphElement, codeElement);
}
